...
const queries = {
  ...
  // `),
  getAlsoBoughtById: companyId => db.any(`
    SELECT alsobought_id FROM alsobought
    WHERE company_id = ${companyId};
  `).then((companyIds) => {
    companyIds = companyIds.map(company => company.alsobought_id);
    const promises = [
      db.any(`SELECT * FROM companies WHERE id IN (${companyIds});`),
      db.any(`SELECT * FROM prices WHERE company_id IN (${companyIds});`),
    ];
